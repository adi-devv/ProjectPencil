...
def _like_rnncell(cell):
    """Checks if a given object is an RNNCell by checking the presence of state_size."""
    return hasattr(cell, "state_size") and hasattr(cell, "output_size")

def linear(input_, output_size, scope=None, reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope or "Linear", reuse=reuse):
...
def rnn_free_run(cell, sequence_length, initial_state, initial_input=None, scope=None):
    with tf.compat.v1.variable_scope(scope or "rnn_free_run"):
        if initial_input is None:
            initial_input = tf.zeros([tf.shape(initial_state)[0], cell.output_size])
        ...
        def loop_fn(time, cell_output, cell_state, loop_state):
            ...
            next_input = tf.cond(
                finished,
                lambda: tf.zeros([tf.shape(initial_state)[0], cell.output_size]),
                lambda: cell_output
            )
